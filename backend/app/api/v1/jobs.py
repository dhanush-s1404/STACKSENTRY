...
async def get_application_count(db: AsyncSession, job_id: UUID) -> int:
    """
    Get total applications for a job.
    Works with SQLAlchemy dynamic relationships.
    """
...
async def list_jobs(
    ...
    for job in jobs:
        app_count = await get_application_count(db, job.id)
...
async def update_job(
    ...
    app_count = await get_application_count(
        db,
        job.id
    )
...
async def delete_job(
    ...
    deleted = await JobService.delete_job(
        db,
        job_id
    )
